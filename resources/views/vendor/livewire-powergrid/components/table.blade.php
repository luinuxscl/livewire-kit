                ...
                @if (isset($setUp['detail']))
                    @foreach ($data as $row)
                        @php
                            ...
                            $class = 'bg-white border-b border-gray-200 hover:bg-gray-100 text-gray-800 transition-colors dark:bg-gray-900 dark:border-gray-800 dark:hover:bg-gray-800 dark:text-gray-200';
                        @endphp
                ...
                @else
                    @foreach ($data as $row)
                        @php
                            ...
                            $class = 'bg-white border-b border-gray-200 hover:bg-gray-100 text-gray-800 transition-colors dark:bg-gray-900 dark:border-gray-800 dark:hover:bg-gray-800 dark:text-gray-200';
                        @endphp
                        <tr wire:key="tr-{{ substr($rowId, 0, 6) }}" class="{{ $class }}">
                            @include('livewire-powergrid::components.row', [
                                'rowIndex' => $loop->index + 1,
                            ])
                        </tr>
